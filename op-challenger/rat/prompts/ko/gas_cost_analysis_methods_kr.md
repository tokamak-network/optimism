...
| **2a** | Proposer | `DisputeGameFactory.create()` | RAT 배포됨, 확률 체크 실패 | **~167,695 가스** | **~188,695 가스** | RAT 확률 체크에서 early return | ✅ 성공 |
| **2b** | Proposer | `DisputeGameFactory.create()` | RAT 배포됨, 트리거 안됨 | **~163,991 가스** | **~184,991 가스** | RAT가 배포되어 있지만 호출되지 않음 | ✅ 성공 |
| **3a** | Proposer | `DisputeGameFactory.create()` + `triggerAttentionTest()` | 확률 통과 + 유효한 챌린저 있음 | **~262,366 가스** | **~283,366 가스** | 게임 생성 + 확률 체크 + 챌린저 선택, 보증금 차감 | ✅ 성공 |
| **3b** | Proposer | `DisputeGameFactory.create()` + `triggerAttentionTest()` | 확률 통과 + 유효한 챌린저 없음 | **~167,895 가스** | **~188,895 가스** | 게임 생성 + 확률 체크 + 조건 검사만, 조용히 무시 | ✅ 성공 |
...
| **RAT 트리거** |  실행함 (유효한 챌린저 있음) vs 실행안함 (조건검사만) | 98,375 vs 3,728-3,904 | 98,375 vs 3,728-3,904 | 25-26배 차이 (추가 intrinsic gas 없음) |
...
| **`triggerAttentionTest()`** | 유효한 챌린저 있음 | **98,375 가스** | 챌린저 선택, 보증금 차감, AttentionInfo 생성 | ✅ 성공 |
| **`triggerAttentionTest()`** | 유효한 챌린저 없음 | **3,728-3,904 가스** | 조용히 무시됨 (가스 절약) | ✅ 성공 |
...
#### **유효한 챌린저 있음 (98,375 가스)**
- **테스트 환경 가스 비용**: 98,375 가스 (intrinsic gas 제외)
- **실제 네트워크 가스 비용**: 98,375 가스 (추가 intrinsic gas 없음 - 컨트랙트 호출)
- **챌린저 선택**: 해시 기반 랜덤 선택
...
#### **유효한 챌린저 없음 (3,728-3,904 가스)**
- **테스트 환경 가스 비용**: 3,728-3,904 가스 (intrinsic gas 제외)
- **실제 네트워크 가스 비용**: 3,728-3,904 가스 (추가 intrinsic gas 없음 - 컨트랙트 호출)
...
- **예상 가스 비용**: ~262,366 가스 (163,991 + 98,375)
- **포함 작업**:
  - 게임 생성 (163,991 가스)
  - 챌린저 선택 (해시 기반 랜덤 선택)
  - 보증금 계산 및 차감
...
- **예상 가스 비용**: ~167,895 가스 (163,991 + 3,904)
- **포함 작업**:
  - 게임 생성 (163,991 가스)
...
6. **조건부 실행**: 유효한 챌린저 없을 때의 최소 오버헤드 (3,728-3,904 가스)

### 최적화 포인트
1. **조건부 실행**: 유효한 챌린저가 없을 때 최소한의 오버헤드 (3,728-3,904 가스)
...
### 주요 비교 지표
1. **게임 생성 기본 비용**: 163,991 가스 (RAT 유무와 관계없음)
2. **RAT 트리거 성공 vs 무시**: 98,375 가스 vs 3,728-3,904 가스 (약 25-26배 차이)
