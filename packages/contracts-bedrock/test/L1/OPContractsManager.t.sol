...
} from "interfaces/universal/IOptimismMintableERC20Factory.sol";
...
contract OPContractsManager_Upgrade_Harness is CommonTest {
    ...
    event Upgraded(
        ...
        ISystemConfig indexed systemConfig,
    ...
    function setUp() public virtual override {
        ...
        delayedWETHPermissionedGameProxy = IDelayedWETH(
            payable(artifacts.mustGetAddress("PermissionedDelayedWETHProxy"))
        );
        delayedWeth = IDelayedWETH(
            payable(artifacts.mustGetAddress("PermissionlessDelayedWETHProxy"))
        );
        permissionedDisputeGame = IPermissionedDisputeGame(
            address(artifacts.mustGetAddress("PermissionedDisputeGame"))
        );
    ...
    function runUpgrade13UpgradeAndChecks(address _delegateCaller) public {
        ...
        IOPCMImplementationsWithoutLockbox.Implementations
            memory impls = IOPCMImplementationsWithoutLockbox(address(deployedOPCM))
                .implementations();
        ...
        // SuperchainConfig contract is upgraded. Separate context to avoid stack too deep.
        {
            ...
            address superchainPAO = IProxyAdmin(EIP1967Helper.getAdmin(address(superchainConfig)))
                .owner();
            vm.etch(superchainPAO, vm.getDeployedCode("test/mocks/Callers.sol:DelegateCaller"));
            DelegateCaller(superchainPAO).dcForward(
                OPCM_ADDRESS,
                abi.encodeCall(
                    IOPContractsManager.upgrade,
                    (new IOPContractsManager.OpChainConfig[](0))
                )
            );
        }
        ...
        bytes32 salt = keccak256(
            abi.encode(
                ...
                string.concat(
                    string(
                        bytes.concat(
                            bytes32(uint256(uint160(address(opChainConfigs[0].systemConfigProxy))))
                        )
                    )
                ),
                "AnchorStateRegistry"
        ...
        IFaultDisputeGame oldFDG = IFaultDisputeGame(
        ...
        DelegateCaller(_delegateCaller).dcForward(
            ...
            abi.encodeCall(IOPContractsManager.upgrade, (opChainConfigs))
        );
        ...
        assertEq(
            ...
            EIP1967Helper.getImplementation(address(l1ERC721Bridge))
        );
        assertEq(
            impls.disputeGameFactoryImpl,
            EIP1967Helper.getImplementation(address(disputeGameFactory))
        );
        assertEq(
            impls.optimismPortalImpl,
            EIP1967Helper.getImplementation(address(optimismPortal2))
        );
        assertEq(
            impls.optimismMintableERC20FactoryImpl,
        ...
        assertEq(
            ...
            EIP1967Helper.getImplementation(address(newAnchorStateRegistryProxy))
        );
        assertEq(
            impls.delayedWETHImpl,
            EIP1967Helper.getImplementation(address(delayedWETHPermissionedGameProxy))
        );
        ...
        if (address(oldFDG) != address(0)) {
            ...
            IFaultDisputeGame newFDG = IFaultDisputeGame(
                address(disputeGameFactory.gameImpls(GameTypes.CANNON))
            );
            // Check that the PermissionlessDisputeGame is upgraded to the expected version,
            ...
            assertEq(
                impls.delayedWETHImpl,
                EIP1967Helper.getImplementation(address(newFDG.weth()))
            );
    ...
    function runUpgrade14UpgradeAndChecks(address _delegateCaller) public {
        ...
        if (_delegateCaller != mainnetPAO) {
            IOPContractsManager.OpChainConfig[]
                memory opmChain = new IOPContractsManager.OpChainConfig[](0);
            ISuperchainConfig superchainConfig = ISuperchainConfig(mainnetPAO);

            address opmUpgrader = IProxyAdmin(EIP1967Helper.getAdmin(address(superchainConfig)))
                .owner();
        ...
        // sanity check
        IPermissionedDisputeGame oldPDG = IPermissionedDisputeGame(
            address(disputeGameFactory.gameImpls(GameTypes.PERMISSIONED_CANNON))
        );
        IFaultDisputeGame oldFDG = IFaultDisputeGame(
            address(disputeGameFactory.gameImpls(GameTypes.CANNON))
    ...
    function runUpgrade15UpgradeAndChecks(address _delegateCaller) public {
        ...
        address newAsrProxy;
        {
            ...
            bytes32 salt = keccak256(
                abi.encode(
                    l2ChainId,
                    string.concat(
                        string(bytes.concat(bytes32(uint256(uint160(address(systemConfig))))))
                    ),
        ...
        DeployUtils.assertInitialized({
            _contractAddress: address(systemConfig),
            _isProxy: true,
            _slot: 0,
            _offset: 0
        });
        ...
        DeployUtils.assertInitialized({
            _contractAddress: address(newAsrProxy),
            _isProxy: true,
            _slot: 0,
            _offset: 0
        });
...
contract OPContractsManager_TestInit is Test {
    ...
    function setUp() public virtual {
        ...
        (blueprints.resolvedDelegateProxy, ) = Blueprint.create(
            vm.getCode("ResolvedDelegateProxy"),
            salt
        );
        (blueprints.permissionedDisputeGame1, blueprints.permissionedDisputeGame2) = Blueprint
            .create(vm.getCode("PermissionedDisputeGame"), salt);
        (blueprints.permissionlessDisputeGame1, blueprints.permissionlessDisputeGame2) = Blueprint
            .create(vm.getCode("FaultDisputeGame"), salt);
        (
        ...
        IOPContractsManager.Implementations memory impls = IOPContractsManager.Implementations({
            ...
            l1ERC721BridgeImpl: DeployUtils.create1({
                ...
                _args: DeployUtils.encodeConstructor(
                    abi.encodeCall(IL1ERC721Bridge.__constructor__, ())
                )
            }),
            optimismPortalImpl: DeployUtils.create1({
                _name: "OptimismPortal2",
                _args: DeployUtils.encodeConstructor(
                    abi.encodeCall(IOptimismPortal2.__constructor__, (1))
                )
            }),
            ethLockboxImpl: DeployUtils.create1({
                _name: "ETHLockbox",
                _args: DeployUtils.encodeConstructor(
                    abi.encodeCall(IETHLockbox.__constructor__, ())
                )
            }),
            systemConfigImpl: DeployUtils.create1({
                _name: "SystemConfig",
                _args: DeployUtils.encodeConstructor(
                    abi.encodeCall(ISystemConfig.__constructor__, ())
                )
            }),
            optimismMintableERC20FactoryImpl: DeployUtils.create1({
                _name: "OptimismMintableERC20Factory",
                _args: DeployUtils.encodeConstructor(
                    abi.encodeCall(IOptimismMintableERC20Factory.__constructor__, ())
                )
            }),
            l1CrossDomainMessengerImpl: DeployUtils.create1({
                _name: "L1CrossDomainMessenger",
                _args: DeployUtils.encodeConstructor(
                    abi.encodeCall(IL1CrossDomainMessenger.__constructor__, ())
                )
            }),
            l1StandardBridgeImpl: DeployUtils.create1({
                _name: "L1StandardBridge",
                _args: DeployUtils.encodeConstructor(
                    abi.encodeCall(IL1StandardBridge.__constructor__, ())
                )
            }),
            disputeGameFactoryImpl: DeployUtils.create1({
                _name: "DisputeGameFactory",
                _args: DeployUtils.encodeConstructor(
                    abi.encodeCall(IDisputeGameFactory.__constructor__, ())
                )
            }),
            anchorStateRegistryImpl: DeployUtils.create1({
                _name: "AnchorStateRegistry",
                _args: DeployUtils.encodeConstructor(
                    abi.encodeCall(IAnchorStateRegistry.__constructor__, (1))
                )
            }),
            delayedWETHImpl: DeployUtils.create1({
                _name: "DelayedWETH",
                _args: DeployUtils.encodeConstructor(
                    abi.encodeCall(IDelayedWETH.__constructor__, (3))
                )
            }),
            mipsImpl: DeployUtils.create1({
                ...
                _args: DeployUtils.encodeConstructor(
                    abi.encodeCall(
                        IMIPS64.__constructor__,
                        (oracle, StandardConstants.MIPS_VERSION)
                    )
                )
            })
        });
        ...
        IOPContractsManager.Implementations memory __opcmImplementations = container
            .implementations();
                        ...
                        (
                            ...
                            IOPContractsManagerDeployer(
                                DeployUtils.createDeterministic({
                                    ...
                                    _args: DeployUtils.encodeConstructor(
                                        abi.encodeCall(
                                            IOPContractsManagerDeployer.__constructor__,
                                            (container)
                                        )
                                    ),
                                    _salt: DeployUtils.DEFAULT_SALT
                            ...
                            IOPContractsManagerUpgrader(
                                DeployUtils.createDeterministic({
                                    ...
                                    _args: DeployUtils.encodeConstructor(
                                        abi.encodeCall(
                                            IOPContractsManagerUpgrader.__constructor__,
                                            (container)
                                        )
                                    ),
                                    _salt: DeployUtils.DEFAULT_SALT
                            ...
                            IOPContractsManagerInteropMigrator(
                                DeployUtils.createDeterministic({
                                    ...
                                    _args: DeployUtils.encodeConstructor(
                                        abi.encodeCall(
                                            IOPContractsManagerInteropMigrator.__constructor__,
                                            (container)
                                        )
                                    ),
                                    _salt: DeployUtils.DEFAULT_SALT
        ...
        vm.mockCall(
            ...
            abi.encodeWithSignature("paused(address)"),
            abi.encode(false)
        );
    ...
    function setupEnvVars() public {
        ...
        vm.setEnv(
            "EXPECTED_SUPERCHAIN_PROXY_ADMIN",
            vm.toString(address(opcm.superchainProxyAdmin()))
        );
    ...
    function createChainContracts(
        uint256 _l2ChainId
    ) internal returns (IOPContractsManager.DeployOutput memory) {
        return
            opcm.deploy(
                IOPContractsManager.DeployInput({
                    ...
                    disputeAbsolutePrestate: Claim.wrap(
                        bytes32(
                            hex"038512e02c4c3f7bdaec27d00edf55b7155e0905301e1a88083e4e0a6764d54c"
                        )
                    ),
                    ...
                    winningChallengerTrackerAddress: address(0)
                })
            );
    }
}
...
contract OPContractsManager_ChainIdToBatchInboxAddress_Test is Test {
    ...
    function setUp() public {
        ...
        OPContractsManager.Implementations memory __opcmImplementations = container
            .implementations();
...
contract OPContractsManager_AddGameType_Test is OPContractsManager_TestInit {
    event GameTypeAdded(
        ...
        IDisputeGame newDisputeGame,
        IDisputeGame oldDisputeGame
    );
    ...
    function test_addGameType_permissioned_succeeds() public {
        ...
        IPermissionedDisputeGame newPDG = IPermissionedDisputeGame(
    ...
    function test_addGameType_permissionless_succeeds() public {
        ...
        // Check the values on the new game type.
        IPermissionedDisputeGame notPDG = IPermissionedDisputeGame(
            address(output.faultDisputeGame)
        );
    ...
    function test_addGameType_unsupportedGameType_reverts() public {
        ...
        IOPContractsManager.AddGameInput[] memory inputs = new IOPContractsManager.AddGameInput[](
            1
        );
        inputs[0] = input;
        (bool success, ) = address(opcm).delegatecall(
            abi.encodeCall(IOPContractsManager.addGameType, (inputs))
        );
            ...
            payable(
                address(
                    DeployUtils.create1({
                        _name: "DelayedWETH",
                        _args: DeployUtils.encodeConstructor(
                            abi.encodeCall(IDelayedWETH.__constructor__, (1))
                        )
                    })
                )
            )
    ...
    function test_addGameType_duplicateGameType_reverts() public {
        ...
        (bool success, bytes memory revertData) = address(opcm).delegatecall(
            abi.encodeCall(IOPContractsManager.addGameType, (inputs))
        );
        assertFalse(success, "addGameType should have failed");
        assertEq(
            bytes4(revertData),
            IOPContractsManager.InvalidGameConfigs.selector,
            "revertData mismatch"
        );
    }
    ...
    function addGameType(
        IOPContractsManager.AddGameInput memory input
    ) internal returns (IOPContractsManager.AddGameOutput memory) {
        IOPContractsManager.AddGameInput[] memory inputs = new IOPContractsManager.AddGameInput[](
            1
        );
        inputs[0] = input;
        ...
        uint256 l2ChainId = IFaultDisputeGame(
            address(
                IDisputeGameFactory(input.systemConfig.disputeGameFactory()).gameImpls(
                    GameType.wrap(1)
                )
            )
        ).l2ChainId();
        ...
        emit GameTypeAdded(
            ...
            IDisputeGame(payable(address(0))),
            IDisputeGame(payable(address(0)))
        );
        (bool success, bytes memory rawGameOut) = address(opcm).delegatecall(
            abi.encodeCall(IOPContractsManager.addGameType, (inputs))
        );
        assertTrue(success, "addGameType failed");

        IOPContractsManager.AddGameOutput[] memory addGameOutAll = abi.decode(
            rawGameOut,
            (IOPContractsManager.AddGameOutput[])
        );
    ...
    function newGameInputFactory(
        bool permissioned
    ) internal view returns (IOPContractsManager.AddGameInput memory) {
        return
            IOPContractsManager.AddGameInput({
                saltMixer: "hello",
    ...
    function assertValidGameType(
        ...
        assertEq(
            ago.faultDisputeGame.gameType().raw(),
        ...
        assertEq(
            ago.faultDisputeGame.maxGameDepth(),
            agi.disputeMaxGameDepth,
            "maxGameDepth mismatch"
        );
        assertEq(ago.faultDisputeGame.splitDepth(), agi.disputeSplitDepth, "splitDepth mismatch");
        assertEq(
            ago.faultDisputeGame.clockExtension().raw(),
            agi.disputeClockExtension.raw(),
            "clockExtension mismatch"
        );
        assertEq(
        ...
        assertEq(
            address(ago.faultDisputeGame.weth()),
            address(ago.delayedWETH),
            "delayedWETH address mismatch"
        );
        assertEq(
        ...
        assertEq(
            chainDeployOutput1.disputeGameFactoryProxy.initBonds(agi.disputeGameType),
            agi.initialBond,
            "bond mismatch"
        );
    }
...
contract OPContractsManager_UpdatePrestate_Test is OPContractsManager_TestInit {
    ...
    function test_updatePrestate_pdgOnlyWithValidInput_succeeds() public {
        ...
        IOPContractsManager.OpChainConfig[] memory inputs = new IOPContractsManager.OpChainConfig[](
            1
        );
        ...
        vm.etch(
            ...
            vm.getDeployedCode("test/mocks/Callers.sol:DelegateCaller")
        );

        // Trigger the updatePrestate function.
        DelegateCaller(proxyAdminOwner).dcForward(
            address(prestateUpdater),
            abi.encodeCall(IOPContractsManager.updatePrestate, (inputs))
        );
        ...
        IPermissionedDisputeGame pdg = IPermissionedDisputeGame(
            address(
                IDisputeGameFactory(chainDeployOutput1.systemConfigProxy.disputeGameFactory())
                    .gameImpls(GameTypes.PERMISSIONED_CANNON)
            )
        );
...
contract OPContractsManager_Upgrade_Test is OPContractsManager_Upgrade_Harness {
    ...
    function test_upgrade_absolutePrestateOverride_succeeds() public {
        ...
        Claim fdgPrestateBefore = IFaultDisputeGame(
            address(disputeGameFactory.gameImpls(GameTypes.CANNON))
        ).absolutePrestate();
        ...
        Claim fdgPrestateAfter = IFaultDisputeGame(
            address(disputeGameFactory.gameImpls(GameTypes.CANNON))
        ).absolutePrestate();
...
contract OPContractsManager_Migrate_Test is OPContractsManager_TestInit {
        ...
        returns (IOPContractsManagerInteropMigrator.MigrateInput memory)
    {
        ...
        IOPContractsManager.OpChainConfig[]
            memory opChainConfigs = new IOPContractsManager.OpChainConfig[](2);
        ...
        return
            IOPContractsManagerInteropMigrator.MigrateInput({
                ...
                startingAnchorRoot: Proposal({
                    root: Hash.wrap(bytes32(hex"ABBA")),
    ...
    function _doMigration(
        ...
        DelegateCaller(proxyAdminOwner).dcForward(
            address(opcm),
            abi.encodeCall(IOPContractsManager.migrate, (_input))
        );
    ...
    function _assertOldGamesZeroed(IDisputeGameFactory _disputeGameFactory) internal view {
        ...
        assertEq(
            address(_disputeGameFactory.gameImpls(GameTypes.SUPER_PERMISSIONED_CANNON)),
            address(0)
        );
    }
    ...
    function test_migrate_withPermissionlessGame_succeeds() public {
        ...
        // Separate context to avoid stack too deep errors.
        {
            ...
            IDisputeGameFactory oldDisputeGameFactory1 = IDisputeGameFactory(
                payable(chainDeployOutput1.systemConfigProxy.disputeGameFactory())
            );
            IDisputeGameFactory oldDisputeGameFactory2 = IDisputeGameFactory(
                payable(chainDeployOutput2.systemConfigProxy.disputeGameFactory())
            );
        ...
        IOptimismPortal2 optimismPortal1 = IOptimismPortal2(
            payable(chainDeployOutput1.systemConfigProxy.optimismPortal())
        );
        IOptimismPortal2 optimismPortal2 = IOptimismPortal2(
            payable(chainDeployOutput2.systemConfigProxy.optimismPortal())
        );
        ...
        assertEq(
            address(optimismPortal1.ethLockbox()),
        ...
        assertTrue(
            ethLockbox.authorizedPortals(optimismPortal1),
            "ETHLockbox does not have portal 1 authorized"
        );
        assertTrue(
            ethLockbox.authorizedPortals(optimismPortal2),
            "ETHLockbox does not have portal 2 authorized"
        );
        ...
        IDisputeGame superPdgImpl = disputeGameFactory.gameImpls(
            GameTypes.SUPER_PERMISSIONED_CANNON
        );
        ISuperPermissionedDisputeGame superPdg = ISuperPermissionedDisputeGame(
            address(superPdgImpl)
        );
    ...
    function test_migrate_mismatchedProxyAdminOwners_reverts() public {
        ...
        _doMigration(
            input,
            OPContractsManagerInteropMigrator
                .OPContractsManagerInteropMigrator_ProxyAdminOwnerMismatch
                .selector
        );
    }
    ...
    function test_migrate_mismatchedAbsolutePrestates_reverts() public {
        ...
        _doMigration(
            input,
            OPContractsManagerInteropMigrator
                .OPContractsManagerInteropMigrator_AbsolutePrestateMismatch
                .selector
        );
    }
    ...
    function test_migrate_mismatchedSuperchainConfig_reverts() public {
        ...
        _doMigration(
            input,
            OPContractsManagerInteropMigrator
                .OPContractsManagerInteropMigrator_SuperchainConfigMismatch
                .selector
        );
    }
}
...
contract OPContractsManager_Deploy_Test is DeployOPChain_TestBase {
    ...
    function toOPCMDeployInput(
        DeployOPChainInput _doi
    ) internal view returns (IOPContractsManager.DeployInput memory) {
        return
            IOPContractsManager.DeployInput({
                roles: IOPContractsManager.Roles({
