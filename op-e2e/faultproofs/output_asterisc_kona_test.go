...
func TestOutputAsteriscKonaGame(t *testing.T) {
	RunTestAcrossVmTypes(t, testOutputAsteriscKonaGame)
}

func testOutputAsteriscKonaGame(t *testing.T, allocType config.AllocType) {
	ctx := context.Background()
	sys, _ := StartFaultDisputeSystem(t, WithAllocType(allocType))
	t.Cleanup(sys.Close)

	disputeGameFactory := disputegame.NewFactoryHelper(t, ctx, sys)
	game := disputeGameFactory.StartOutputAsteriscKonaGame(ctx, "sequencer", 4, common.Hash{0x01})
	arena := createOutputGameArena(t, sys, game)
	testCannonGame(t, ctx, arena, &game.SplitGameHelper)
}

func TestOutputAsteriscKona_ChallengeAllZeroClaim(t *testing.T) {
	RunTestAcrossVmTypes(t, testOutputAsteriscKonaChallengeAllZeroClaim)
}

func testOutputAsteriscKonaChallengeAllZeroClaim(t *testing.T, allocType config.AllocType) {
	...
	disputeGameFactory := disputegame.NewFactoryHelper(t, ctx, sys)
	game := disputeGameFactory.StartOutputAsteriscKonaGame(ctx, "sequencer", 3, common.Hash{})
	arena := createOutputGameArena(t, sys, game)
	testCannonChallengeAllZeroClaim(t, ctx, arena, &game.SplitGameHelper)
}

func TestOutputAsteriscKona_PublishAsteriscKonaRootClaim(t *testing.T) {
	...
	RunTestsAcrossVmTypes(t, tests, func(t *testing.T, allocType config.AllocType, test TestCase) {
		...
		disputeGameFactory := disputegame.NewFactoryHelper(t, ctx, sys)
		game := disputeGameFactory.StartOutputAsteriscKonaGame(ctx, "sequencer", test.disputeL2BlockNumber, common.Hash{0x01})
		game.DisputeLastBlock(ctx)
		game.LogGameData(ctx)

		game.StartChallenger(ctx, "Challenger",
			challenger.WithPrivKey(sys.Cfg.Secrets.Alice),
			challenger.WithAsteriscKona(t, sys))
...
func TestOutputAsteriscKonaDisputeGame(t *testing.T) {
	type TestCase struct {
		name             string
	...
	RunTestsAcrossVmTypes(t, tests, func(t *testing.T, allocType config.AllocType, test TestCase) {
		...
		disputeGameFactory := disputegame.NewFactoryHelper(t, ctx, sys)
		game := disputeGameFactory.StartOutputAsteriscKonaGame(ctx, "sequencer", 1, common.Hash{0x01, 0xaa})
...
func TestOutputAsteriscKonaDefendStep(t *testing.T) {
	RunTestAcrossVmTypes(t, testOutputAsteriscKonaDefendStep)
}

func testOutputAsteriscKonaDefendStep(t *testing.T, allocType config.AllocType) {
	ctx := context.Background()
	sys, _ := StartFaultDisputeSystem(t, WithAllocType(allocType))
	t.Cleanup(sys.Close)
...
func TestOutputAsteriscKonaStepWithLargePreimage(t *testing.T) {
	RunTestAcrossVmTypes(t, testOutputAsteriscKonaStepWithLargePreimage)
}

func testOutputAsteriscKonaStepWithLargePreimage(t *testing.T, allocType config.AllocType) {
	ctx := context.Background()
	...
	// This forces kona-host to load a large preimage.
	...
	game := disputeGameFactory.StartOutputAsteriscKonaGame(ctx, "sequencer", l2BlockNumber, common.Hash{0x01, 0xaa})
	require.NotNil(t, game)
...
func TestOutputAsteriscKonaStepWithPreimage_nonExistingPreimage(t *testing.T) {
	type TestCase struct {
		name         string
	...
	RunTestsAcrossVmTypes(t, tests, func(t *testing.T, allocType config.AllocType, testcase TestCase) {
		...
		testAsteriscKonaPreimageStep(t, allocType, conf, false, testcase.opts...)
	}, WithTestName(testName))
}

func TestOutputAsteriscKonaStepWithPreimage_nonExistingBlobPreimage(t *testing.T) {
	type TestCase struct {
		blobOffset    uint32
		blobSkipCount int
	}
	...
	RunTestsAcrossVmTypes(t, testCases, func(t *testing.T, allocType config.AllocType, testcase TestCase) {
		...
		testAsteriscKonaPreimageStep(t, allocType, conf, false, disputegame.SkipNPreimageLoads(testcase.blobSkipCount))
	}, WithTestName(testName))
}

func TestOutputAsteriscKonaStepWithPreimage_existingPreimage(t *testing.T) {
	...
	RunTestAcrossVmTypes(t, func(t *testing.T, allocType config.AllocType) {
		conf := utils.PreimageOptConfigForType(oppreimage.Keccak256KeyType)
		testAsteriscKonaPreimageStep(t, allocType, conf, true)
	})
}

func testAsteriscKonaPreimageStep(t *testing.T, allocType config.AllocType, preimageOptConfig utils.PreimageOptConfig, preloadPreimage bool, opts ...disputegame.FindPreimageStepOpt) {
	ctx := context.Background()
...
func TestOutputAsteriscKonaProposedOutputRootValid(t *testing.T) {
	RunTestAcrossVmTypes(t, testOutputAsteriscKonaProposedOutputRootValid_AttackWithCorrectTrace)
}

func testOutputAsteriscKonaProposedOutputRootValid_AttackWithCorrectTrace(t *testing.T, allocType config.AllocType) {
	ctx := context.Background()
	sys, _ := StartFaultDisputeSystem(t, WithAllocType(allocType))
	t.Cleanup(sys.Close)

	disputeGameFactory := disputegame.NewFactoryHelper(t, ctx, sys)
	game := disputeGameFactory.StartOutputAsteriscKonaGameWithCorrectRoot(ctx, "sequencer", 1)
	arena := createOutputGameArena(t, sys, game)
	testCannonProposalValid_AttackWithCorrectTrace(t, ctx, arena, &game.SplitGameHelper)
}

func TestOutputAsteriscKonaProposedOutputRootValid_DefendWithCorrectTrace(t *testing.T) {
	RunTestAcrossVmTypes(t, testOutputAsteriscKonaProposedOutputRootValid_DefendWithCorrectTrace)
}

func testOutputAsteriscKonaProposedOutputRootValid_DefendWithCorrectTrace(t *testing.T, allocType config.AllocType) {
	ctx := context.Background()
	sys, _ := StartFaultDisputeSystem(t, WithAllocType(allocType))
	t.Cleanup(sys.Close)
...
func TestOutputAsteriscKonaPoisonedPostState(t *testing.T) {
	RunTestAcrossVmTypes(t, testOutputAsteriscKonaPoisonedPostState)
}

func testOutputAsteriscKonaPoisonedPostState(t *testing.T, allocType config.AllocType) {
	ctx := context.Background()
	sys, _ := StartFaultDisputeSystem(t, WithAllocType(allocType))
	t.Cleanup(sys.Close)
	...
	game := disputeGameFactory.StartOutputAsteriscKonaGame(ctx, "sequencer", 1, common.Hash{0xaa})
	arena := createOutputGameArena(t, sys, game)
